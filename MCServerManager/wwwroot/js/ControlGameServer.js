...
function checkUserList(element) {
	if (usersInfo.version == element.playersListVersion) {
		return;
	}

	usersInfo.version = element.playersListVersion;
	queryHandling(`${pathPage}/GetUserList`, function (element) {
		usersInfo.userList = element;

		$("#user-count").html(`Количество игроков: ${element.length}`);
